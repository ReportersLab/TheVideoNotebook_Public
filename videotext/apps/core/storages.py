...
class TVNS3UploadBackend(AbstractUploadBackend):
    ...
    def update_filename(self, request, filename):
        ...
        self._user = request.user.username
    ...
    def upload_chunk(self, chunk):
        ...
        self._mp.upload_part_from_file(file, self._counter)
    ...
    def setup(self, filename):
        ...
        if not self._s3_key:
            ...
            self._s3_name = '{0}{1}/{2}-{3}'.format(settings.AWS_UPLOAD_FOLDER, self._user, datetime.now().strftime('%m-%d-%Y-%H-%M-%S'), filename)
            self._s3_key = self._bucket.new_key(self._s3_name)
        
        self._mp = self._bucket.initiate_multipart_upload(self._s3_name)
